<section id="skill" class="skill-section">
   <div class="container-fluid">
   <div class="row">
      <h1 class="text-white skill-sub ">Technical Expertise</h1>
      <p class="text-white skill-sub2">A showcase of my technical, creative, and analytical abilities focused on building reliable and scalable web solutions.</p>
   </div>
   <div class="skill_sub_section">
      <h3 class="skill-sub3">Programming & Markup</h3>
      <hr class="text-white my-4">
      <div class="skills-wrap">
         <span class="skill-btn toggle-btn">HTML</span>
         <span class="skill-btn toggle-btn">CSS</span>
         <span class="skill-btn toggle-btn">JAVASCRIPT</span>
         <span class="skill-btn toggle-btn">PHP</span>
         <span class="skill-btn toggle-btn">BOOTSTRAP</span>
      </div>
   </div>
   <div class="skill_tools">
      <h3 class="skill-sub3">Tools</h3>
      <hr class="text-white my-4">
      <div class="skills-wrap">
        <span class="skill-btn toggle-btn">WordPress CMS</span>
         <span class="skill-btn toggle-btn">Visual Studio Code</span>
         <span class="skill-btn toggle-btn">XAMPP</span>
         <span class="skill-btn toggle-btn">GitHub</span>

      </div>
   </div>
   <div class="skill_tools">
      <h3 class="skill-sub3">Technical Skills</h3>
      <hr class="text-white my-4">
      <div class="skills-wrap">
         <span class="skill-btn toggle-btn">WordPress Development</span>
         <span class="skill-btn toggle-btn">Custom Theme Development</span>
         <span class="skill-btn toggle-btn">WooCommerce Setup</span>
         <span class="skill-btn toggle-btn">Elementor Page Builder</span>
         
      </div>
   </div>
   <!-- <div class="skill_tools">
      <h3 class="skill-sub3">Platforms</h3>
      <hr class="text-white my-4">
      <div class="skills-wrap">
         <span class="skill-btn toggle-btn">Wordpress</span>
         <span class="skill-btn toggle-btn">GitHub</span>
         
      </div>
   </div> -->
   <div class="skill_tools">
      <h3 class="skill-sub3">Soft Skills</h3>
      <hr class="text-white my-4">
      <div class="skills-wrap">
         <span class="skill-btn toggle-btn">Teamwork</span>
         <span class="skill-btn toggle-btn">Time management</span>
         <span class="skill-btn toggle-btn">Positive attitude</span>
         <span class="skill-btn toggle-btn">Creativity</span>
         <span class="skill-btn toggle-btn">Learning attitude</span>
      </div>
   </div>
   <div class="skill_tools">
      <h3 class="skill-sub3">Languages</h3>
      <hr class="text-white my-4">
      <div class="skills-wrap">
         <span class="skill-btn toggle-btn">Marathi</span>
         <span class="skill-btn toggle-btn">English</span>
         <span class="skill-btn toggle-btn">Hindi</span>
      </div>
   </div>
</section>